module.exports.register = function(handlebars) {
    ...
    handlebars.registerHelper('buildMenuOptions', function(arr, val) {
        var ret = '';
        var entries = val.data.root.sections;
        for (var ii=0; ii < entries.length; ii++) {
            if (entries[ii].depth === 2) {
                ret += '<a id="styleguideDropdown-' + entries[ii].header + '" onclick="updateUrl(\'' + entries[ii].referenceURI + '\',\'' + entries[ii].header + '\')" tabindex="0">' + entries[ii].header + '</a>';
            }
        }
        return new handlebars.SafeString(ret);
    });
    ...
    handlebars.registerHelper('buildTabs', function(arr, val) {
        ...
        for (var ii=0; ii < entries.length; ii++) {
            if (entries[ii].depth === 2) {
                ret += '<li class="tab" id="styleguideTabs-' + entries[ii].header + '"><a onclick="updateUrl(\'' + entries[ii].referenceURI + '\',\'' +entries[ii].header + '\')" tabindex="0"><div class="tab__heading" title="' + entries[ii].header + '">' + entries[ii].header + '</div></a></li>';
            }
        }
        return new handlebars.SafeString(ret);
    });
    ...
    handlebars.registerHelper('extractCategory', function(arg1, arg2, options) {
        return arg1.split('-')[0];
    });
